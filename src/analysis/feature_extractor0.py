...
class Feature_Extractor:
    ...
    def extract_time_domain_features(self):
        ...
        # 计算信号熵（基于幅度概率分布）
        amp_normalized = cp.abs(self.signal) / cp.sum(cp.abs(self.signal))  # 归一化幅度
        amp_normalized = amp_normalized[amp_normalized > 0]  # 排除零值避免log(0)
        signal_entropy = -cp.sum(amp_normalized * cp.log2(amp_normalized))
        # 基本统计特征
        features = {
            # 幅度特征
    ...
    def extract_frequency_features(self, num_bins=None):
        ...
        # 计算频谱熵
        power_normalized = power_spectrum / total_power  # 归一化功率谱
        power_normalized = power_normalized[power_normalized > 0]  # 排除零值
        spectral_entropy = -cp.sum(power_normalized * cp.log2(power_normalized))

        # 计算频谱中心（重心频率）
        spectral_centroid = cp.sum(freq_axis * power_spectrum) / total_power if total_power != 0 else 0

        # 计算频谱带宽（基于二阶矩）
        if total_power != 0:
            spectral_variance = cp.sum(((freq_axis - spectral_centroid) ** 2) * power_spectrum) / total_power
            spectral_bandwidth = cp.sqrt(spectral_variance)
        else:
...
if __name__ == "__main__":
    ...
    features_data = np.load('../result/features/features.npz')

    # 查看所有特征及其值
    print("\n所有特征数据：")
    for feature_name in features_data.files:
        print(f"{feature_name}: {features_data[feature_name]}")
    ...
    # }
    #
    # custom_extractor = Feature_Extractor(
    #     gpu_id=0,
